#include "skid_steer_planning/a_star_costmap_planner.hpp"

namespace skid_steer_planning
{
AStarCostmapPlanner::~AStarCostmapPlanner(){}

AStarCostmapPlanner::AStarCostmapPlanner(const std::string &name):Node(name){
    ...
    map_subscriber = create_subscription<nav_msgs::msg::OccupancyGrid>("/costmap/costmap", rclcpp::QoS(rclcpp::KeepLast(10)).transient_local().reliable(), 
                     std::bind(&AStarCostmapPlanner::map_callback, this, std::placeholders::_1));
    
    pose_subscriber = create_subscription<geometry_msgs::msg::PoseStamped>("/goal_pose", 10, 
                      std::bind(&AStarCostmapPlanner::goal_callback, this, std::placeholders::_1));
    
    path_publisher = create_publisher<nav_msgs::msg::Path>("/a_star/path", 10);
    map_publisher = create_publisher<nav_msgs::msg::OccupancyGrid>("/a_star/visited_map", 10);


}
void AStarCostmapPlanner::map_callback(const nav_msgs::msg::OccupancyGrid::SharedPtr map){
    this->map = map;
...
void AStarCostmapPlanner::goal_callback(const geometry_msgs::msg::PoseStamped::SharedPtr pose){
    if (!map){
...
nav_msgs::msg::Path AStarCostmapPlanner::plan_trajectory(const geometry_msgs::msg::Pose &start, const geometry_msgs::msg::Pose &goal){
    ...
    start_node.heuristic = ChebyshevDistance(start_node, goal_node);
    ...
    while (!pending_nodes.empty() && rclcpp::ok()){
        ...
        for (const auto &dir : explore_vecinity){
            ...
            if(std::find(visited_nodes.begin(), visited_nodes.end(), new_node) == visited_nodes.end() 
                    && PoseOnMap(new_node) && map->data.at(PoseToCell(new_node)) >= 0 && map->data.at(PoseToCell(new_node)) < 99)
            {
                new_node.cost = active_node.cost + 1 + map->data.at(PoseToCell(new_node));
                new_node.heuristic = ChebyshevDistance(new_node, goal_node);
...
bool AStarCostmapPlanner::PoseOnMap(const GraphNode &node){
    return node.x >= 0 && node.x < static_cast<int>(map->info.width) && node.y >= 0 && node.y < static_cast<int>(map->info.height);
}

unsigned int AStarCostmapPlanner::PoseToCell(const GraphNode &node){
    return node.y * map->info.width + node.x;
}


geometry_msgs::msg::Pose AStarCostmapPlanner::GridToWorld(const GraphNode &node){
...
GraphNode AStarCostmapPlanner::WorldToGrid(const geometry_msgs::msg::Pose &pose){
...
double AStarCostmapPlanner::ChebyshevDistance(const GraphNode &node, const GraphNode &goal_node){
    double dx = static_cast<double>(abs(goal_node.x - node.x));
    double dy = static_cast<double>(abs(goal_node.y - node.y));
...
int main(int argc, char **argv){
    ...
    auto node = std::make_shared<skid_steer_planning::AStarCostmapPlanner>("a_star_planner");
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}