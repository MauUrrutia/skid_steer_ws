#include "skid_steer_planning/dijkstra_planner.hpp"

namespace skid_steer_planning
{
DijkstraPlanner::~DijkstraPlanner(){}

DijkstraPlanner::DijkstraPlanner(const std::string &name):Node(name){
...
void DijkstraPlanner::goal_callback(const geometry_msgs::msg::PoseStamped::SharedPtr pose){
    if (!map){
...
nav_msgs::msg::Path DijkstraPlanner::plan_trajectory(const geometry_msgs::msg::Pose &start, const geometry_msgs::msg::Pose &goal){
...
int main(int argc, char **argv){
    ...
    auto node = std::make_shared<skid_steer_planning::DijkstraPlanner>("dijkstra_planner");
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}