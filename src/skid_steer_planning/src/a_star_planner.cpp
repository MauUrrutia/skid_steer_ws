#include "skid_steer_planning/a_star_planner.hpp"

namespace skid_steer_planning
{
AStarPlanner::~AStarPlanner(){}

AStarPlanner::AStarPlanner(const std::string &name):Node(name){

    tf_buffer = std::make_unique<tf2_ros::Buffer>(get_clock()); 
    tf_listener = std::make_shared<tf2_ros::TransformListener>(*tf_buffer);
    map_subscriber = create_subscription<nav_msgs::msg::OccupancyGrid>("/map", rclcpp::QoS(rclcpp::KeepLast(10)).transient_local().reliable(), 
...
void AStarPlanner::goal_callback(const geometry_msgs::msg::PoseStamped::SharedPtr pose){
    if (!map){
...
nav_msgs::msg::Path AStarPlanner::plan_trajectory(const geometry_msgs::msg::Pose &start, const geometry_msgs::msg::Pose &goal){
...
double AStarPlanner::ChebyshevDistance(const GraphNode &node, const GraphNode &goal_node){
...
int main(int argc, char **argv){
    ...
    auto node = std::make_shared<skid_steer_planning::AStarPlanner>("a_star_planner");
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}