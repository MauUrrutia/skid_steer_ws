...
class dijkstra_costmap_planner(Node):
    def __init__(self):
        super().__init__("dijkstra_costmap_planner")

        self.map_qos = QoSProfile(depth = 10, )
        self.map_qos.durability = DurabilityPolicy.TRANSIENT_LOCAL
        self.map_subscriber = self.create_subscription(OccupancyGrid, "/costmap/costmap", self.map_callback, self.map_qos)
    ...
    def plan(self, start, goal):
        ...
        while not pending_nodes.empty() and rclpy.ok():
            ...
            for dir_x, dir_y in explore_directions:
                ...
                if new_node not in visited_nodes and self.pose_to_map(new_node) and 0 <= self.map.data[self.pose_to_cell(new_node)] < 99:
                    new_node.cost = active_node.cost + 1 + self.map.data[self.pose_to_cell(new_node)]
