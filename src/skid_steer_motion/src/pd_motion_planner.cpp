#include "skid_steer_motion/pd_motion_planner.hpp"

namespace skid_steer_motion
{
PdMotionPlanner::~PdMotionPlanner(){}
PdMotionPlanner::PdMotionPlanner(const std::string &name) : Node(name), kp(2.0), kd(0.1), step_size(0.2), max_linear_vel(3.0), max_angular_vel(1.5),
                                                            prev_linear_error(0.0), prev_angular_error(0.0)
{
...
void PdMotionPlanner::ControlLoop(){
    ...
    cmd_vel.linear.x = std::clamp(kp * linear_error + kd * linear_error_derivative, -max_linear_vel, max_linear_vel);
    cmd_vel.angular.z = std::clamp(kp * angular_error + kd * angular_error_derivative, -max_angular_vel, max_angular_vel);
...
int main(int argc, char **argv){
    ...
    auto node = std::make_shared<skid_steer_motion::PdMotionPlanner>("pd_motion_planner");
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}