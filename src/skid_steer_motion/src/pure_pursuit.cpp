...
// PurePursuit::PurePursuit(const std::string &name) : Node(name),  look_ahead_distance(0.5), max_linear_vel(3.0), max_angular_vel(1.5)
// {
...
//     check_point_pose_publisher = create_publisher<geometry_msgs::msg::PoseStamped>("/pure_pursuit/check_point_pose", 10);
...
//     auto checkpoint_pose = GetCheckpointPose(robot_pose_stamped);

//     double dx = checkpoint_pose.pose.position.x - robot_pose_stamped.pose.position.x;
//     double dy = checkpoint_pose.pose.position.y - robot_pose_stamped.pose.position.y;
//     double dz = checkpoint_pose.pose.position.z - robot_pose_stamped.pose.position.z;
...
//     check_point_pose_publisher->publish(checkpoint_pose);
    
//     tf2::Transform robot_tf, checkpoint_pose_tf, chekpoint_pose_robot_tf;
//     tf2::fromMsg(robot_pose_stamped.pose, robot_tf);
//     tf2::fromMsg(checkpoint_pose.pose, checkpoint_pose_tf);

//     chekpoint_pose_robot_tf = robot_tf.inverse() * checkpoint_pose_tf;
//     tf2::toMsg(chekpoint_pose_robot_tf, checkpoint_pose.pose);
//     auto curvature = GetCurvature(checkpoint_pose.pose);

//     geometry_msgs::msg::Twist cmd_vel;
//     cmd_vel.linear.x = max_linear_vel;
//     cmd_vel.angular.z = curvature * max_angular_vel;
...
// geometry_msgs::msg::PoseStamped PurePursuit::GetCheckpointPose(const geometry_msgs::msg::PoseStamped &robot_pose){
//     auto checkpoint_pose = global_plan.poses.back();
...
//             break;
//     }
//     return checkpoint_pose;    
// }

// double PurePursuit::GetCurvature(const geometry_msgs::msg::Pose &checkpoint_pose){
//     const double L = (checkpoint_pose.position.x * checkpoint_pose.position.x) + (checkpoint_pose.position.y * checkpoint_pose.position.y);
//     if(L > 0.001)
//         return 2 * checkpoint_pose.position.y / L;
//     else
//         return 0.0;
// }
...
//     auto node = std::make_shared<skid_steer_motion::PurePursuit>("pure_pursuit_motion_planner");
//     rclcpp::spin(node);
//     rclcpp::shutdown();
//     return 0;
// }
...
void PurePursuit::ControlLoop(){
    ...
    // Obtener la transformación del robot
    ...
    // Transformar el plan al marco del robot si es necesario
    if (!TransformPlan(robot_pose.header.frame_id)) {
        RCLCPP_ERROR(get_logger(), "Unable to transform plan to robot frame");
        return;
    }

    // Obtener punto de mira
    ...
    // Publicar punto de mira para visualización
    ...
    // Calcular curvatura y velocidades
