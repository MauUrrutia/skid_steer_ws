#include "skid_steer_motion/pi_motion_planner.hpp"

namespace skid_steer_motion
{
PiMotionPlanner::~PiMotionPlanner(){}
PiMotionPlanner::PiMotionPlanner(const std::string &name) : Node(name), 
kp(2.0), 
ki(0.01),
...
prev_angular_error(0.0)
{
    ...
    path_subscriber = create_subscription<nav_msgs::msg::Path>("/a_star/path", 10, 
                      std::bind(&PiMotionPlanner::PathCallback, this, std::placeholders::_1));

    cmd_vel_publisher = create_publisher<geometry_msgs::msg::Twist>("cmd_vel", 10);
    next_pose_publisher = create_publisher<geometry_msgs::msg::PoseStamped>("pi/next_pose", 10);

    tf_buffer = std::make_shared<tf2_ros::Buffer>(get_clock());
    tf_listener = std::make_shared<tf2_ros::TransformListener>(*tf_buffer);

    control_loop = create_wall_timer(std::chrono::milliseconds(100), std::bind(&PiMotionPlanner::ControlLoop, this));
    last_cycle_time = get_clock()->now();
}
    
void PiMotionPlanner::PathCallback(const nav_msgs::msg::Path::SharedPtr path){
    global_plan = *path;
}   

void PiMotionPlanner::ControlLoop(){
    ...
    cmd_vel.linear.x = std::clamp(kp * linear_error + ki * integral_linear_error, -max_linear_vel, max_linear_vel);
    cmd_vel.angular.z = std::clamp(kp * angular_error + ki * integral_angular_error, -max_angular_vel, max_angular_vel);
...
bool PiMotionPlanner::TransformPlan(const std::string &frame){
...
geometry_msgs::msg::PoseStamped PiMotionPlanner::GetNextPose(const geometry_msgs::msg::PoseStamped &robot_pose){
...
int main(int argc, char **argv){
    ...
    auto node = std::make_shared<skid_steer_motion::PiMotionPlanner>("pi_motion_planner");
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}